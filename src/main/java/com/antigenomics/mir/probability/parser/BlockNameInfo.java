...
public final class BlockNameInfo {
    ...
    public static BlockNameInfo fromBlockName(String blockName) {
        var splitBlock = blockName.split(PlainTextHierarchicalModelUtils.REGEX_CONDITIONAL_SEPARATOR);
        return new BlockNameInfo(
                blockName,
                Arrays.asList(splitBlock[0].split(PlainTextHierarchicalModelUtils.VARIABLE_SEPARATOR)),
                splitBlock.length > 1 ? Arrays.asList(splitBlock[1].split(PlainTextHierarchicalModelUtils.VARIABLE_SEPARATOR)) :
